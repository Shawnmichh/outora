...
<<<<<<< HEAD
        ...
        has_movie_pref = self._should_include_movie_theaters(preferences)
        has_food_pref = self._should_include_food_stops(preferences)
        
        for stop in validated_all:
            is_protected = False
            if has_food_pref and stop.get('category') == 'food':
                is_protected = True
            if has_movie_pref and stop.get('is_movie_theater'):
                is_protected = True
                
            if is_protected:
                protected_stops.append(stop)
            else:
                unprotected_stops.append(stop)
                
        # Reorder so protected stops are at the front (guaranteed to survive slice)
        validated_all_reordered = protected_stops + unprotected_stops
        
        # Take final recommendations
        recommendations = validated_all_reordered[:max_recommendations]
        
        # FINAL INTENT VALIDATION: Ensure protected categories survived
        final_food_count = sum(1 for s in recommendations if s.get('category') == 'food')
        final_movie_count = sum(1 for s in recommendations if s.get('is_movie_theater'))
        
        # Force-insert missing food
        if has_food_pref and final_food_count == 0:
            best_food = next((s for s in unique_places if s.get('category') == 'food'), None)
            if best_food:
                logger.warning('FINAL VALIDATION: Force-inserting missing food stop: %s', best_food['name'])
                if len(recommendations) >= max_recommendations:
                    # Remove the lowest priority non-protected stop (usually explore)
                    non_protected = [i for i, s in enumerate(recommendations) if s.get('category') != 'food' and not s.get('is_movie_theater')]
                    if non_protected:
                        recommendations.pop(non_protected[-1])
                recommendations.append(best_food)
                
        # Force-insert missing movie theater
        if has_movie_pref and final_movie_count == 0:
            best_movie = next((s for s in unique_places if s.get('is_movie_theater')), None)
            if best_movie:
                logger.warning('FINAL VALIDATION: Force-inserting missing movie theater: %s', best_movie['name'])
                if len(recommendations) >= max_recommendations:
                    # Remove the lowest priority non-protected stop
                    non_protected = [i for i, s in enumerate(recommendations) if s.get('category') != 'food' and not s.get('is_movie_theater')]
                    if non_protected:
                        recommendations.pop(non_protected[-1])
...
<<<<<<< HEAD
        ...
        if vibe == 'romantic':
            # Romantic vibe prioritizes food/ambience/evening, less sightseeing
            quotas = {
                'food': max_food,
                'explore': max(0, int(remaining_stops * 0.15)),      # 15% - Reduced sightseeing
                'activity': max(1, int(remaining_stops * 0.40)),     # 40% - High (movies, hangouts)
                'shopping': max(0, int(remaining_stops * 0.15)),     # 15%
                'culture': max(0, int(remaining_stops * 0.10)),      # 10%
                'nature': max(1, int(remaining_stops * 0.20)),       # 20% - Increased (sunsets, walks)
            }
        else:
...
<<<<<<< HEAD
            ...
            if max_food > 0:
                quotas['food'] = max(max_food, quotas.get('food', 0))
                
            # If scaling caused total to exceed max (due to floor/ceil issues), cap non-essential
            excess = sum(quotas.values()) - total_stops
            for category in ['explore', 'shopping', 'culture']:
                if excess <= 0:
                    break
        ...
        # NEW: Guarantee at least 1 activity stop if movie theaters should be included
...
<<<<<<< HEAD
        ...
        if preferences:
            max_movie_theaters = self._get_max_movie_theaters(preferences)
        
        # Count existing movie theaters in collected_by_category
        existing_movie_count = 0
        if collected_by_category:
            for category_stops in collected_by_category.values():
...
<<<<<<< HEAD
                ...
                filter_prefs['target_category'] = target_category
                filtered_places = self.place_filter.filter_places(places, filter_prefs)
                
                if len(filtered_places) < len(places):
                    logger.info(
                        'Filtered %d/%d places for category "%s" (removed irrelevant types)',
                        len(filtered_places),
                        len(places),
                        target_category,
                    )
                ...
                for place in filtered_places:
                    ...
                    # Check if this is a movie theater
                    is_movie_theater = self.place_filter.is_movie_theater(place)
                    
                    # ENFORCE MOVIE THEATER LIMIT
                    if is_movie_theater:
                        # Count movie theaters already collected in this batch
                        current_movie_count = existing_movie_count + sum(1 for s in collected if s.get('is_movie_theater'))
                        
                        if current_movie_count >= max_movie_theaters:
                            logger.info(
                                'Skipping movie theater "%s" - already have %d/%d movie theaters',
                                place.get('name'),
                                current_movie_count,
                                max_movie_theaters,
                            )
                            continue  # Skip this movie theater
                        
                        logger.info('Found movie theater: %s (count: %d/%d)', place.get('name'), current_movie_count + 1, max_movie_theaters)
...
<<<<<<< HEAD
        ...
        # vibe from overwriting 'restaurant' → 'food' assignments with 'dining'.
        type_based_category = self._category_from_types(place.get('types', []))
        
        # type_based_category defaults to 'explore' when nothing matches.
        # Use the vibe map only when we got the generic 'explore' default AND
        # the vibe has a meaningful override (so cultural/nightlife vibes still work).
        if type_based_category == 'explore' and vibe in VIBE_CATEGORY_MAP:
            vibe_category = VIBE_CATEGORY_MAP[vibe]
            # Only override if vibe_category is non-explore (adds real information)
            if vibe_category not in ('explore', None):
...
<<<<<<< HEAD
        ...
        # to prevent sightseeing spam when other categories fail to return enough results.
        if vibe == 'romantic':
...
<<<<<<< HEAD
            ...
            def get_sort_score(s):
                ...
                # Give a massive bonus to movie theaters if they should be included, 
                # guaranteeing they get picked first for their category
                if s.get('is_movie_theater') and self._should_include_movie_theaters(preferences):
                    base_score += 100
                return base_score
                
            # Take up to quota, sorted by rating + movie preference bonus
...
<<<<<<< HEAD
    ...
    def _should_include_movie_theaters(self, preferences: dict[str, Any]) -> bool:
        """
        Determine if movie theaters should be included based on movie preference.
        
        NEW: Movie-aware theater inclusion logic
        - Returns False if movie_preference is 'no'
        - Returns True if movie_preference is 'yes'
        - Returns True if movie_preference is 'maybe' AND vibe matches entertainment/nightlife
        - Returns True if movie_preference is 'maybe' AND outing is evening/long duration
        
        Returns:
            True if movie theaters should be included, False otherwise
        """
        movie_pref = preferences.get('movie_preference', 'maybe')
        ...
        # Maybe - check contextual appropriateness
        if movie_pref == 'maybe':
            # Check vibe
            vibe = preferences.get('outing_vibe', '')
            if vibe in ['nightlife', 'fun', 'romantic', 'family']:
                return True
            
            # Check time of day (evening outings)
            start_time_str = preferences.get('start_time', '10:00')
            try:
                start_hour = int(start_time_str.split(':')[0])
                if start_hour >= 17:  # After 5 PM
                    return True
            except (ValueError, AttributeError, IndexError):
                pass
            
            # Check duration (long outings)
            duration_hours = self._calculate_outing_duration(preferences)
            if duration_hours >= 6.0:  # 6+ hour outings
                return True
            
            # Default: no movies for 'maybe' if not contextually appropriate
            return False
        
        # Default: no movies
        return False
    
    def _get_max_movie_theaters(self, preferences: dict[str, Any]) -> int:
        """
        Get maximum number of movie theaters allowed per itinerary.
        
        HARD LIMIT: Maximum 1 movie theater per itinerary
        
        Returns:
            Maximum number of movie theaters (0 or 1)
        """
