...
class OutingPlanGenerator:
    ...
    def _stops_from_recommendations(
        ...
        for rec in recommendations:
            ...
            if rec.get('is_movie_theater'):
                ...
                stop['is_movie_theater'] = True  # Preserve flag for frontend
                try:
                    enhanced_stop = self.movie_intelligence.enhance_theater_stop(stop, preferences)
                    stop = enhanced_stop
    ...
    def _apply_ai_optimization(
        ...
        # 1. Pull movie theaters to the very end
        movies = [s for s in stops if s.get('is_movie_theater')]
        
        # 2. Extract food stops
        food = [s for s in stops if s.get('category') == 'food' and not s.get('is_movie_theater')]
        
        # 3. Extract all other stops
        others = [s for s in stops if not s.get('is_movie_theater') and s.get('category') != 'food']
        
        # Sort others so daytime activities (explore/nature) come before evening activities (nightlife)
        def get_category_weight(stop):
            cat = stop.get('category', '')
            if cat in ['explore', 'nature']:
                return 1
            if cat in ['shopping', 'culture']:
                return 2
            if cat in ['activity']:
                return 3
            if cat in ['nightlife']:
        ...
        others.sort(key=get_category_weight)
        
        optimized = []
        
        vibe = preferences.get('outing_vibe', '')
        if vibe == 'romantic' and len(food) == 1:
            # For a single romantic meal (dinner), put it after most activities but before nightlife/movies
            day_activities = [s for s in others if get_category_weight(s) < 5]
            night_activities = [s for s in others if get_category_weight(s) == 5]
            
            optimized.extend(day_activities)
            optimized.extend(food)
            optimized.extend(night_activities)
        else:
            # Distribute food evenly among activities
            if not others:
                optimized.extend(food)
            else:
                interval = len(others) // (len(food) + 1)
                idx = 0
                for f in food:
                    idx += interval + 1
                    others.insert(min(idx, len(others)), f)
                optimized = others
                
        # Movie is always the grand finale
        optimized.extend(movies)
