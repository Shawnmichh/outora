...
<<<<<<< HEAD
        <div className="border-t border-[#e2e5ea] bg-white px-5 py-4">
          ...
          {hasRoute && legDurations.length > 0 && (
            <div className="mb-3 flex flex-wrap items-center gap-y-1 text-xs text-[#374151]">
              <span className="mr-1 font-semibold text-[#0f172a]">Travel time:</span>
              {legDurations.map((dur, i) => (
                <span key={i} className="flex items-center gap-1">
                  <span className="text-[#6b7280]">
                    {resolvedStops[i]?.stop.name.split(' ')[0]}
                  </span>
                  <svg className="h-3 w-3 shrink-0 text-[#9ca3af]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2} aria-hidden>
                    <path strokeLinecap="round" strokeLinejoin="round" d="M17.25 8.25L21 12m0 0l-3.75 3.75M21 12H3" />
                  </svg>
